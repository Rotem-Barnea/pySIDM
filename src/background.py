...
class BackgroundDistribution:
    ...
    @classmethod
    def from_files(
        cls, total_mass: Quantity['mass'], files: pd.DataFrame | None = None, **kwargs: Unpack[nsphere.FileParams]
    ) -> Self:
        """Initialize a background mass distribution object from input files, i.e. `NSphere` output.

        Parameters:
            total_mass: The total mass of the background distribution.
        ...
        M = np.vstack([lattice.values_on_lattice_point_cumsum(lattice(d)) for d in data]) * total_mass / data.shape[1]
