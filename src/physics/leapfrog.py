...
@njit(parallel=True)
def fast_step(r:NDArray[np.float64],v:NDArray[np.float64],M:NDArray[np.float64],dt:float,regulator:float=0,max_minirounds:int=100,
...
def step(r:units.Quantity['length'],v:units.Quantity['velocity'],M:units.Quantity['mass'],dt:units.Quantity['time'],
         regulator:units.Quantity['length']=default_params['regulator'],max_minirounds:int=default_params['max_minirounds'],
    ...
    _r,_v = fast_step(r=r.value,v=v.value,M=M.value,dt=dt.value,regulator=regulator.value,max_minirounds=max_minirounds,
                      r_convergence_threshold=r_convergence_threshold.value,vr_convergence_threshold=vr_convergence_threshold.value)
