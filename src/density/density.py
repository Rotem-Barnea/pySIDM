...
from ..constants import time,length,velocity,mass,density,G_units,energy,f_units
...
class Density:
    def __init__(self,Rmin:units.Quantity['length']=1e-4*units.kpc,Rmax:units.Quantity['length']|None=None,Rs:units.Quantity['length']=1*units.kpc,
        ...
        self.Mtot:units.Quantity['mass'] = Mtot.to(mass)
        self.title = 'Density'
        self.unit_mass:units.Quantity['mass'] = unit_mass.to(mass)
        self.Rs:units.Quantity['length'] = Rs.to(length)
        self.Rvir:units.Quantity['length'] = Rvir.to(length)
        self.Rmin:units.Quantity['length'] = Rmin.to(length)
        self.Rmax:units.Quantity['length'] = (Rmax or 85*self.Rs).to(length)
  ...
  - particle mass = {self.unit_mass:.3e}"""
    ...
    def Tdyn(self) -> units.Unit:
        if 'Tdyn' not in self.memoization:
            self.memoization['Tdyn'] = units.def_unit('Tdyn',np.sqrt(self.Rs**3/(constants.G.to(G_units)*self.Mtot)).to(time),doc=f'{self.title} dynamic time')
    ...
    def rho(self,r:units.Quantity['length']) -> units.Quantity['mass density']:
        return cast(units.Quantity['mass density'],self.calculate_rho(r.to(length).value,self.rho_s.value,self.Rs.value,self.Rvir.value)*density)
    ...
    def rho_r2(self,r:units.Quantity['length']) -> units.Quantity['linear density']:
        return self.rho(r)*r.to(length)**2
    ...
    def spherical_rho_integrate(self,r:units.Quantity['length'],use_rho_s:bool=True) -> units.Quantity['mass']:
        ...
        integral = utils.fast_spherical_rho_integrate(np.atleast_1d(r.to(length).value),self.calculate_rho,rho_s,self.Rs.value,self.Rvir.value)
        return units.Quantity(integral,mass)

    def M(self,r:units.Quantity['length']) -> units.Quantity['mass']:
        scalar_input = np.isscalar(r)
        M = self.spherical_rho_integrate(r)
        if scalar_input:
            return units.Quantity(np.array(M)[0],mass)
        return M
    ...
    def calculate_rho_scale(self) -> units.Quantity['mass density']:
        return self.Mtot/(self.spherical_rho_integrate(self.Rmax,False)[0]*length**3)

    def Phi(self,r:units.Quantity['length']) -> units.Quantity['energy']:
        if 'Phi' not in self.memoization:
            r_grid = self.geomspace_grid
            M_grid = self.M(r_grid)
            Phi_grid = -constants.G.to(G_units).value*scipy.integrate.cumulative_trapezoid(y=M_grid.value/r_grid.value**2,x=r_grid.value,initial=0)
            Phi_grid -= Phi_grid[-1]
            Phi_grid *= -1
            self.memoization['Phi'] = scipy.interpolate.interp1d(r_grid.value,Phi_grid,kind='cubic',bounds_error=False,fill_value=(0,0))
        return units.Quantity(self.memoization['Phi'](r.to(length).value),energy)

    @property
    def Phi_grid(self) -> units.Quantity['energy']:
    ...
    @property
    def Phi0(self) -> units.Quantity['energy']:
        if 'Phi0' not in self.memoization:
            self.memoization['Phi0'] = self.Phi(self.Rmax)
        return self.memoization['Phi0']

    def Psi(self,r:units.Quantity['length']) -> units.Quantity['energy']:
        return cast(units.Quantity['energy'],self.Phi0 - self.Phi(r))

    @property
    def Psi_grid(self) -> units.Quantity['energy']:
    ...
    def pdf(self,r:units.Quantity['length']) -> FloatOrArray:
        ...
        return self.memoization['pdf'](r.to(length).value)
    ...
    def cdf(self,r:units.Quantity['length']) -> FloatOrArray:
        if 'cdf' not in self.memoization:
            self.memoization['cdf'] = scipy.interpolate.interp1d(self.geomspace_grid.value,self.mass_cdf(self.geomspace_grid),
                                                                 kind='cubic',bounds_error=False,fill_value=(0,1))
        return self.memoization['cdf'](r.to(length).value)
    ...
    def quantile_function(self,p:FloatOrArray) -> units.Quantity['length']:
        ...
        return units.Quantity(self.memoization['quantile_function'](p),length)

    def Psi_to_r(self,Psi:units.Quantity['energy']) -> units.Quantity['length']:
        if 'Psi_to_r' not in self.memoization:
            ...
            self.memoization['Psi_to_r'] = scipy.interpolate.interp1d(Psi_grid,r_grid,kind='cubic',bounds_error=False,
                                                                      fill_value=(self.Rmin.value,self.Rmax.value))
        return units.Quantity(self.memoization['Psi_to_r'](Psi.to(energy).value),length)

    def Psi_to_rho(self,Psi:units.Quantity['energy']) -> units.Quantity['mass density']:
        return self.rho(self.Psi_to_r(Psi))

    def drhodPsi(self,Psi:units.Quantity['energy']) -> units.Quantity:
        return utils.quantity_derivate(Psi,self.Psi_to_rho)

    def drho2dPsi2(self,Psi:units.Quantity['energy']) -> units.Quantity:
        return utils.quantity_derivate2(Psi,self.Psi_to_rho)

    def calculate_f(self,E:units.Quantity['energy']) -> units.Quantity[f_units]:
        scalar_input = np.isscalar(E)
        Psi = cast(units.Quantity['energy'],np.linspace(self.Psi_grid.min(),E.to(energy).max(),1000))
        drho2dPsi2 = self.drho2dPsi2(Psi)
        integral = units.Quantity(np.zeros_like(np.atleast_1d(E.value)),drho2dPsi2.unit*np.sqrt(1*energy))
        ...
        if scalar_input:
            integral = integral[0]
        return 1/(self.unit_mass*np.sqrt(8)*np.pi**2)*(self.drhodPsi(0*energy)/np.sqrt(E)+integral)

    def E(self,r:units.Quantity['length'],v:units.Quantity['velocity']) -> units.Quantity['energy']:
        return cast(units.Quantity['energy'],self.Psi(r)-v**2/2)

    def f(self,E:units.Quantity['energy']) -> units.Quantity[f_units]:
        if 'f' not in self.memoization:
            E_grid = cast(units.Quantity['energy'],np.linspace(0,self.Psi_grid.max(),int(1e3))[1:])
        ...
        return units.Quantity(self.memoization['f'](E.to(energy).value),f_units)
...
## Roll initial setup
    ...
    def roll_v(self,r:units.Quantity['length'],num:int=1000) -> units.Quantity['velocity']:
        E_grid = cast(units.Quantity['energy'],np.linspace(0,self.Psi_grid.max(),int(num))[1:])
        return units.Quantity(self.roll_v_fast(self.Psi(r).to(energy).value,E_grid.value,f_grid=self.calculate_f(E_grid).value,num=num),velocity)
...
##Plots
    ...
    def add_plot_R_markers(self,ax:Axes,ymax:float,x_units:UnitLike) -> Axes:
    ...
    def plot_rho(self,r_start:units.Quantity['length']|None=None,r_end:units.Quantity['length']|None=1e4*units.kpc,density_units:UnitLike=density,
                 length_units:UnitLike=length,fig:Figure|None=None,ax:Axes|None=None) -> tuple[Figure,Axes]:
        fig,ax = utils.setup_plot(fig,ax,title='Density distribution (rho)',xlabel=f'Radius [{length_units:latex}]',ylabel=f'Density [{density_units:latex}]')
    ...
    def plot_radius_distribution(self,r_start:units.Quantity['length']|None=None,r_end:units.Quantity['length']|None=None,cumulative:bool=False,
                                 plot_units:UnitLike=length,fig:Figure|None=None,ax:Axes|None=None) -> tuple[Figure,Axes]:
        title = 'Particle cumulative range distribution (cdf)' if cumulative else 'Particle range distribution (pdf)'
        fig,ax = utils.setup_plot(fig,ax,title=title,xlabel=f'Radius [{plot_units:latex}]',ylabel='Density')
